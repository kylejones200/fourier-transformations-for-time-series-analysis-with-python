...
def generate_synthetic_signal(time: np.ndarray, freq1: float, freq2: float, amplitude2: float=0.5) -> np.ndarray:
...
def filter_noise(fft_result: np.ndarray, frequencies: np.ndarray, threshold: float) -> np.ndarray:
...
def add_noise(signal: np.ndarray, noise_level: float=50, seed: int | None=None) -> np.ndarray:
    """Add Gaussian noise to signal."""
    if seed is not None:
        np.random.seed(seed)
    return signal + np.random.normal(0, noise_level, len(signal))

def plot_time_series(time: np.ndarray, signal: np.ndarray, output_path: Path, title: str, xlabel: str='Time', ylabel: str='Amplitude', plot: bool=False):
    """Plot time series"""
    if not plot:
        return
...
def plot_frequency_domain(frequencies: np.ndarray, fft_result: np.ndarray, output_path: Path, title: str, plot: bool=False):
...
def plot_noise_filtering(time: np.ndarray, noisy_signal: np.ndarray, filtered_signal: np.ndarray, output_path: Path, plot: bool=False):
    ...
    ax.plot(time, noisy_signal, label='Noisy Signal', color='#8B6F9E', linewidth=1.2, alpha=0.5)
