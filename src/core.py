...
def generate_synthetic_signal(
    time: np.ndarray, freq1: float, freq2: float, amplitude2: float = 0.5
) -> np.ndarray:
    """Generate synthetic signal with two frequencies."""
    return np.sin(2 * np.pi * freq1 * time) + amplitude2 * np.sin(
        2 * np.pi * freq2 * time
    )


def compute_fft(
    signal: np.ndarray, sample_rate: float = 1.0
...
def filter_noise(
    fft_result: np.ndarray, frequencies: np.ndarray, threshold: float
) -> np.ndarray:
...
def add_noise(
    signal: np.ndarray, noise_level: float = 50, seed: int = None
) -> np.ndarray:
...
def plot_frequency_domain(
    frequencies: np.ndarray, fft_result: np.ndarray, output_path: Path, title: str
):
