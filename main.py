...
def main():
    parser = argparse.ArgumentParser(
        description="Fourier Transformations for Time Series"
    )
    ...
    if config["analysis"]["run_synthetic"]:
        ...
        time = np.linspace(
            0,
            config["data"]["synthetic"]["time_end"],
    ...
    if config["analysis"]["run_noise_filtering"]:
        ...
        noisy_signal = add_noise(
            y, config["data"]["noise"]["level"], config["data"]["synthetic"]["seed"]
        )
        frequencies, fft_result_noisy = compute_fft(noisy_signal, sample_rate=1)
        fft_filtered = filter_noise(
            fft_result_noisy, frequencies, config["data"]["noise"]["threshold"]
        )
        filtered_signal = inverse_fft(fft_filtered)
        plot_noise_filtering(
            time, noisy_signal, filtered_signal, output_dir / "noise_filtering.png"
        )
